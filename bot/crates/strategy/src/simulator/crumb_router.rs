...
use crate::{
    constants::{
        crumb_ROUTER_ADDRESS, crumb_ROUTER_CODE, crumb_ROUTER_CONTROLLER, WETH_ADDRESS, WETH_FUND_AMT,
    },
    tx_utils::crumb_router_interface::{
...
async fn evaluate_sandwich_revenue(
    ...
    inject_crumb_router_code(&mut fork_db);
    ...
    evm.env.tx.caller = *crumb_ROUTER_CONTROLLER;
    evm.env.tx.transact_to = TransactTo::Call(*crumb_ROUTER_ADDRESS);
    ...
    let result = match evm.transact_commit() {
        Ok(result) => result,
        Err(e) => return Err(anyhow!("[crumbRouter: EVM ERROR] frontrun: {:?}", e)),
    };
    let output = match result {
        ...
        ExecutionResult::Revert { output, .. } => {
            return Err(anyhow!("[crumbRouter: REVERT] frontrun: {:?}", output))
        }
        ExecutionResult::Halt { reason, .. } => {
            return Err(anyhow!("[crumbRouter: HALT] frontrun: {:?}", reason))
        }
    };
    let (_frontrun_out, backrun_in) = match ingredients.get_target_pool() {
        UniswapV2(_) => match decode_swap_v2_result(output.into()) {
            ...
            Err(e) => {
                return Err(anyhow!(
                    "[crumbRouter: FailedToDecodeOutput] frontrun: {:?}",
                    e
                ))
            }
        },
        UniswapV3(_) => match decode_swap_v3_result(output.into()) {
            Ok(output) => output,
            Err(e) => return Err(anyhow!("crumbRouter: FailedToDecodeOutput: {:?}", e)),
        },
    };
    ...
    let result = match evm.transact_commit() {
        Ok(result) => result,
        Err(e) => return Err(anyhow!("[crumbRouter: EVM ERROR] backrun: {:?}", e)),
    };
    let output = match result {
        ExecutionResult::Success { output, .. } => match output {
            Output::Call(o) => o,
        ...
        ExecutionResult::Revert { output, .. } => {
            return Err(anyhow!("[crumbRouter: REVERT] backrun: {:?}", output))
        }
        ExecutionResult::Halt { reason, .. } => {
            return Err(anyhow!("[crumbRouter: HALT] backrun: {:?}", reason))
        }
    };
...
fn inject_crumb_router_code(db: &mut CacheDB<SharedBackend>) {
    // insert crumbRouter bytecode
    let crumb_router_info = AccountInfo::new(
        rU256::ZERO,
        0,
        Bytecode::new_raw((*CRUMB_ROUTER_CODE.0).into()),
    );
    db.insert_account_info(*CRUMB_ROUTER_ADDRESS, crumb_router_info);

    // insert and fund crumbRouter controller (so we can spoof)
    let controller_info = AccountInfo::new(*WETH_FUND_AMT, 0, Bytecode::default());
    db.insert_account_info(*CRUMB_ROUTER_CONTROLLER, controller_info);

    // fund crumbRouter with 200 weth
    let slot = keccak256(&abi::encode(&[
        abi::Token::Address((*CRUMB_ROUTER_ADDRESS).into()),
